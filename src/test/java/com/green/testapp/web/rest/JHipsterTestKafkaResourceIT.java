package com.green.testapp.web.rest;
...
import com.green.testapp.config.KafkaProperties;
...
class JHipsterTestKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/j-hipster-test-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/j-hipster-test-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
